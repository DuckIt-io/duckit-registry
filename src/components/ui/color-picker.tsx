...
export const ColorPicker = ({
  ...
  const [saturation, setSaturation] = useState(color.saturationl() || 0)
  const [lightness, setLightness] = useState(color.lightness() || 0)
  const [alpha, setAlpha] = useState(color.alpha() * 100)
  const [mode, setMode] = useState('hex')

  // Sync internal state when the controlled value changes
  useEffect(() => {
    if (value) {
